...
if(isset($_POST['btn-insert-time'])){
	...
	if($countEmpty === 0){
		...
		if($stmt->execute()){
			...
			$sql = "SELECT timetable.timetable_id
					FROM timetable 
					WHERE timetable.user_id = ? ORDER BY timetable.timetable_id DESC LIMIT 1;";
			$stmt = $conn->prepare($sql);
			$stmt->bind_param('s', $_SESSION['user_id']);
			$stmt->execute();
			$resultA = $stmt->get_result();
			$resultB = $resultA->fetch_assoc();
			
			$sql = "INSERT INTO location_timetable(timetable_id, location) 
					VALUES(?, ?);";
			$stmt = $conn->prepare($sql);
			$stmt->bind_param('ss', $resultB['timetable_id'], $locationtime);
			$stmt->execute();
...
if(isset($_POST['delete-btn-confirm'])){
	...
	$sqlDeletetime3 = "DELETE
						FROM location_timetable
						WHERE location_timetable.timetable_id = ?;";
	$stmt = $conn->prepare($sqlDeletetime3);
	$stmt->bind_param('s', $d);
	$stmt->execute();
...
if(isset($_POST['request-btn-confirm'])){
	...
	echo '<script type="text/javascript">
					swal({
					  title: "คุณต้องการจองเวลานี้ใช่หรือไม่?",
					  text: "คลิกยืนยันหากต้องการส่งคำร้องขอจองเวลา",
					  icon: "warning",
					  buttons: true,
					  dangerMode: true,
					})
					.then((willDelete) => {
					  if (willDelete) {
						swal("คุณได้ส่งคำร้องขอจองเวลาเรียบร้อยแล้ว", {
						  icon: "success",
						});
					  } else {
					  }
